function rPoly = validate(obj,varargin)
	...
	Kron_delta = Operator(eye(MN),obj.modes);

	% prime operator acts on vectors by Ah = h'
	Idprime = Operator(diag(repmat([1,1:obj.modes(1)-1],1,obj.modes(2))),obj.modes);
	...
	Idshift = Operator([shift_matrix(end,:);shift_matrix(1:end-1,:)],obj.modes);

	% define finite part of DF as 4 Operators (components of the Jacobian)
	DF(1,1) = Idprime; %DxF1
	...
	for j = 1:2
		for k = 1:2
			A(j,k) = Operator(A_matrix(1+(j-1)*MN:j*MN,1+(k-1)*MN:k*MN),obj.modes);
		end
	end
...
%             Y_2 = y2.norm;
	...
	for j = 1:2
		for k = 1:2
			I_ADF(j,k) = Operator(Id_ADF(1+(j-1)*MN:j*MN,1+(k-1)*MN:k*MN),obj.modes);
		end
	end
